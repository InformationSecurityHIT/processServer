...
def compute_curvature(image, sigma):
    ...
    # 1. constructs the 2D gaussian filter "h" given the window size构造给定窗口大小的二维高斯滤波器“h”
    t1 = time.time()
    ...
    t2 = time.time()
    # 2. calculates first and second derivatives of "G" with respect to "X"计算“G”相对于“X”的一阶和二阶导数
    ...
    hxy = ((X * Y) / (sigma ** 8)) * G
    t3 = time.time()
    # 3. calculates derivatives w.r.t. to all directions计算所有方向的导数w.r.t
    ...
    t4 = time.time()
    # [Step 1-1] Calculation of curvature profiles[步骤1-1]曲率剖面的计算
